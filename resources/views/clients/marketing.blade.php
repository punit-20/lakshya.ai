...
@section('title', 'AI Creative Builder')
...
<div class="marketing-wrapper">
    <div class="section-title-wrapper">
        <h1 style="font-size: 1.75rem; font-weight: 700; letter-spacing: -0.5px;">AI Creative Builder</h1>
        <p style="color: var(--text-muted); font-size: 0.9rem; margin-top: 0.25rem;">
            Generate high-converting digital marketing posts containing product descriptions and affiliate marketing links.
        </p>
    </div>
        ...
        <div class="form-section">
            <div class="card">
                <div class="card-header-flex">
                    <h2 style="font-size: 1.1rem; font-weight: 700;">Affiliate Campaign Settings</h2>
                    <span class="badge badge-qualified">Active Workspace</span>
                </div>
                ...
                <form id="marketingGeneratorForm" onsubmit="generateCampaign(event)">
                    <div class="form-group">
                        ...
                        <textarea id="business_description" class="form-control" placeholder="Describe the business work, its offerings, or specific campaign focus (e.g. 'Hand-crafted botanical silver rings made from recycled metals.')" required></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="target_audience" class="form-control" placeholder="e.g. Sterling silver enthusiasts, nature lovers, green fashionistas" required>
                    </div>

                    <div class="form-group">
                        <label for="cta">Call to Action / Affiliate Marketing Link <span style="color: #f43f5e;">*</span></label>
                        <input type="text" id="cta" class="form-control" placeholder="e.g. Click here to buy and get 15% off: http://amzn.to/example-affiliate-link" required>
                    </div>

                    <button type="submit" id="generateBtn" class="btn btn-primary" style="width: 100%; margin-top: 0.5rem; height: 46px;">
                        <span id="btnText">⚡ Generate Affiliate Campaign Post</span>
            ...
            <div class="card" style="height: 100%; display: flex; flex-direction: column;">
                ...
                <div id="previewEmptyState" style="flex-grow: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1rem; padding: 3rem 1rem; border: 2px dashed var(--border-color); border-radius: 12px;">
                    ...
                    <p style="color: var(--text-muted); font-size: 0.9rem; text-align: center; max-width: 250px;">
                        Enter campaign details and your affiliate link on the left to generate the campaign creative package.
                    </p>
                </div>
                ...
                <div id="previewActiveState" style="display: none; flex-grow: 1; flex-direction: column; gap: 1.25rem;">
                    ...
                    <div class="mock-post-card">
                        <div class="mock-post-header">
                            <div class="mock-avatar">{{ substr($client->name, 0, 1) }}</div>
                            <div class="mock-user-info">
                                <span class="mock-username">{{ $client->name }}</span>
                                <span class="mock-timestamp">Just now • <span id="previewPlatformTag" class="platform-tag twitter">Twitter / X</span></span>
                            </div>
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 0.5rem;">
                            <label style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; color: var(--text-muted);">Campaign Copy & Affiliate link</label>
    ...
    <div class="modal-content" style="max-width: 500px;">
        ...
        <div class="modal-body" style="padding-top: 0.5rem;">
            <p style="color: var(--text-muted); font-size: 0.85rem; line-height: 1.45;">
                Deploying affiliate marketing campaign creative package to client targets and social media API outboxes.
            </p>
            ...
            <div id="launchSuccessMessage" style="display: none; margin-top: 1rem; padding: 1rem; border-radius: 12px; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.2); text-align: center; flex-direction: column; gap: 0.5rem; align-items: center;">
                ...
                <h4 style="font-weight: 700; color: #34d399; font-size: 0.95rem;">Affiliate Campaign Successfully Launched!</h4>
                <p style="color: var(--text-muted); font-size: 0.8rem; line-height: 1.4;">
                    Your post has been distributed, logged in target outbox audit files, and alerts registered on client dashboards.
                </p>
            </div>
        </div>
...
<script>
    ...
    function generateCampaign(event) {
        ...
        fetch('{{ route("client.marketing.generate") }}', {
            method: 'POST',
            headers: {
        ...
        .catch(err => {
            ...
            alert('Failed to generate campaign creative. Please try again.');
        });
    }
        ...
        runStep('step1', 800, () => {
            ...
            runStep('step2', 800, () => {
                ...
                runStep('step3', 900, () => {
                    ...
                    runStep('step4', 1000, () => {
                        // Submit backend call
                        fetch('{{ route("client.marketing.launch") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
